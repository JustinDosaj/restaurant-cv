...
- **RX.CUSTOM-HOOK-NAME** — Custom hooks start with `use`, are named exports (function declaration), one per file, camelCase filename matching the hook (`useAuth.ts → useAuth`); no kebab-case hook filenames.
...
- **RX.FEATURE-BOUNDARY** — Code under `client/src/features/<feature>/` may import its own feature (`./` siblings or `@/features/<feature>/...`) and the shared layers, never another feature; promote anything two features need to `client/src/{components,hooks,lib,types}` or `shared/`. ESLint-enforced (`eslint.config.js`), including a ban on parent-relative imports inside a feature so the boundary stays lintable. The property/company/group presentational components (`client/src/components/{properties,companies}/`), their pure helpers, and `client/src/api/{properties,companies}.api.ts` are the promoted shared layer; feature takeovers/hosts (Data, Insights) wrap them.
...
- **RX.NO-RAW-FETCH** — Never call `fetch` directly in a component or hook; call an api module (`client/src/features/<feature>/api/`, or `client/src/api/` for the shared cross-feature modules), which wraps `apiRequest`, usually via TanStack Query. ESLint-enforced (`eslint.config.js`); raw fetch is legal only inside api modules and `@/lib/queryClient`.
